...
export class GuessNumberModule extends Module {
    ...
    handleGuess() {
        ...
        if (userGuess === this.targetNumber) {
            this.addMessageToContainer(
                `Попытка ${this.currentAttempt}: Вы угадали! Загаданное число было ${this.targetNumber}.`,
                "correct"
            );
            this.endGame();
        } else if (this.currentAttempt >= this.attempts) {
            this.addMessageToContainer(
                `Попытка ${this.currentAttempt}: Вы исчерпали все попытки. Загаданное число было ${this.targetNumber}.`,
                "incorrect"
            );
            this.endGame();
        } else {
            if (userGuess < this.targetNumber) {
                this.addMessageToContainer(
                    `Попытка ${this.currentAttempt}: ${userGuess} — Слишком мало!`,
                    "incorrect"
                );
            } else {
                this.addMessageToContainer(
                    `Попытка ${this.currentAttempt}: ${userGuess} — Слишком много!`,
                    "incorrect"
                );
            }
    ...
    addMessageToContainer(message, status) {
        const messageEntry = document.createElement("p");
        messageEntry.textContent = message;
        messageEntry.classList.add(status);
        this.guessesContainer.prepend(messageEntry);
    }
